...
assert(file_exists(__DIR__ . '/artifacts/dts-file-transfer.zip'));
assert(file_exists(__DIR__ . '/artifacts/dts-file-transfer.phar'));
...
$client->post(
    $uploadUrl . '/assets?name=dts-file-transfer.zip',
    array('Content-Type' => 'application/zip'),
    fopen(__DIR__ . '/artifacts/dts-file-transfer.zip', 'r')
)->send();

$client->post(
    $uploadUrl . '/assets?name=dts-file-transfer.phar',
    array('Content-Type' => 'application/phar'),
    fopen(__DIR__ . '/artifacts/dts-file-transfer.phar', 'r')
)->send();
